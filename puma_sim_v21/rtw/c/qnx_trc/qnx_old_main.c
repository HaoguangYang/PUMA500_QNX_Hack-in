	...
	for (;;) {
		...
		if (pid == 0) {
			if (msg.code == TIMER_PULSE_CODE) {
			    ...
			    rtExtModeOneStep(rtmGetRTWExtModeInfo(S), rtGetNumSampleTimes(S), (boolean_T *)&rtmGetStopRequested(S));
...
int_T main(int_T argc, const char_T *argv[])
{
    ...
    rtExtModeWaitForStartPkt(rtmGetRTWExtModeInfo(S), rtGetNumSampleTimes(S), 
	...
	//printf("4\n");
		...
		//printf("6\n");
		if (fmod((float)rtmGetT(S),0.005) == 0)
			printf("here ?%9.6f\n", (float) rtmGetT(S));
...
printf("between\n");
    ...
    rtExtModeShutdown(rtGetNumSampleTimes(S));
