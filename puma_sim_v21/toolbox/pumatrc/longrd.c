/*
 * longrd.c: float read block for generic control board
 *
 */
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
	...
	case POSITION6: // POSITION6 -- read all encoders
		robot_longrd(POSITION6, channel, &tmpArray);
		for (i=0;i<6;i++)
			*output++ = tmpArray[i];
		break;
	case POSITION: // POSITION
		robot_longrd(POSITION, channel, &tmp);
		*output = tmp;
		break;
	case INDEX: // INDEX
		robot_longrd(INDEX, channel, &tmp);
		*output = tmp;
		break;
	case STATUS: //STATUS
		robot_longrd(STATUS, channel, &tmp);
		*output = tmp;
		break;
	case POT: // POT
		robot_longrd(POT, channelG, &tmp);
		*output = tmp;
		break;
	case POT6: // POT6 -- read all ADCs
		robot_longrd(POT6, channel, &tmpArray);
		for (i=0;i<6;i++)
			*output++ = tmpArray[i];
		break;
	default:
	...
	if (ssGetSimMode(S) == SS_SIMMODE_NORMAL) {
		mexPrintf("\nlong read: mdlOutput (option %d channel %d)\n",option, channel);
	}
#endif
}
...
#if defined(RT)
	/* generated code calls function to initialize the servotogo driver */
	printf("\nlong read module: termination\n");
	ssSetIWorkValue(S,0,NULL);
	ssSetIWorkValue(S,1,NULL);
#elif defined(MATLAB_MEX_FILE)
	/* during simulation, just print a message */
	if (ssGetSimMode(S) == SS_SIMMODE_NORMAL) {
		mexPrintf("\nlong read.c: Simulated termination\n");
	}
#endif
}
