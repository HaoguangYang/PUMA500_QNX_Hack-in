/*
 * shortwr.c: short write block for control board
 *
 */
...
#define S_FUNCTION_NAME shortwr
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
	...
	switch(option) {
	case TORQUE6: // TORQUE6 -- output to all DACs
		for (i=0;i<6;i++)
			tmpArray[i] = (int) *val[i];
		robot_shortwr(TORQUE6, channel, tmpArray);
		break;
	case TORQUE: // TORQUE
		tmp = (int) *val[0];
		robot_shortwr(TORQUE, channel, &tmp);
		break;
	case POSITION6: // POSITION6
		for (i=0;i<6;i++)
			tmpArray[i] = (int) *val[i];
		robot_shortwr(POSITION6, channel, tmpArray);
		break;
	case POSITION: // POSITION
		tmp = (int) *val[0];
		robot_shortwr(POSITION, channel, &tmp);
		break;
	default:
		printf("short write error: option failed\n");
	}

#elif defined(MATLAB_MEX_FILE)
	/* during simulation, just print a message */
	if (ssGetSimMode(S) == SS_SIMMODE_NORMAL) {
		mexPrintf("\nshort write: mdlOutput\n");
	}
#endif
}
...
#if defined(RT)
	/* generated code calls function to initialize the servotogo driver */
	printf("\nshort write module: termination\n");
	ssSetIWorkValue(S,0,NULL);
	ssSetIWorkValue(S,1,NULL);
#elif defined(MATLAB_MEX_FILE)
	/* during simulation, just print a message */
	if (ssGetSimMode(S) == SS_SIMMODE_NORMAL) {
		mexPrintf("\nshort write: Simulated termination\n");
	}
#endif
}
