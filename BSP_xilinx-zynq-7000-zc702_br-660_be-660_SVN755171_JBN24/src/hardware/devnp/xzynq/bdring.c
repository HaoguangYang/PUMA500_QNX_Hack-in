...
int xzynq_bdring_create(xzynq_bdring_t *ring_ptr, uint32_t phys_addr,
		uint32_t virt_addr, uint32_t alignment, unsigned bd_count)
{
	unsigned i;
	...
	ring_ptr->separation = (sizeof(xzynq_bd_t) + (alignment - 1))
			& ~(alignment - 1);
	 ...
	 */
	memset((void *) virt_addr, 0, (ring_ptr->separation * bd_count));
	...
	ring_ptr->length = ring_ptr->high_bd_addr - ring_ptr->base_bd_addr
			+ ring_ptr->separation;
	ring_ptr->all_cnt = bd_count;
	ring_ptr->free_cnt = bd_count;
	ring_ptr->free_head = (xzynq_bd_t *) virt_addr;
	ring_ptr->pre_head = (xzynq_bd_t *) virt_addr;
	ring_ptr->hw_head = (xzynq_bd_t *) virt_addr;
	ring_ptr->hw_tail = (xzynq_bd_t *) virt_addr;
	ring_ptr->post_head = (xzynq_bd_t *) virt_addr;
	ring_ptr->bda_restart = (xzynq_bd_t *) phys_addr;
		...
		uint8_t direction)
{
	 ...
	 */
	for (i = 0, cur_bd = (uint32_t) ring_ptr->base_bd_addr; i
			< ring_ptr->all_cnt; i++, cur_bd
			+= ring_ptr->separation) {
		memcpy((void *) cur_bd, src_bd_ptr, sizeof(xzynq_bd_t));
	...
	if (direction == XZYNQ_EMACPS_RECV)
		xzynq_bd_set_rx_wrap(cur_bd);
	else
		xzynq_bd_set_tx_wrap(cur_bd);
		...
		xzynq_bd_t *bdset_ptr, int is_rx)
{
	...
	/* Make sure we are in sync with xzynq_bdring_alloc() */
	...
	for (i = 0; i < num_bd; i++) {
		if (is_rx == XZYNQ_EMACPS_RECV) {
			xzynq_bd_write(cur_bd_ptr, XZYNQ_EMACPS_BD_STAT_OFFSET, 0);

			regval = xzynq_bd_read(cur_bd_ptr, XZYNQ_EMACPS_BD_ADDR_OFFSET);
			regval &= ~XZYNQ_EMACPS_RXBUF_NEW_MASK;
			xzynq_bd_write(cur_bd_ptr, XZYNQ_EMACPS_BD_ADDR_OFFSET, regval);
		} else {
		...
		cur_bd_ptr = xzynq_bdring_next(ring_ptr, cur_bd_ptr);
	}
...
unsigned xzynq_bdring_from_hw_tx(xzynq_bdring_t *ring_ptr, unsigned bd_limit,
		xzynq_bd_t **bdset_ptr)
{
	xzynq_bd_t *cur_bd_ptr;
	uint32_t bd_str = 0;
	...
	while (bd_count < bd_limit) {
		...
		bd_str = xzynq_bd_read(cur_bd_ptr, XZYNQ_EMACPS_BD_STAT_OFFSET);

		if (sop == 0) {
			if (bd_str & XZYNQ_EMACPS_TXBUF_USED_MASK)
				sop = 1;
			else
				break;
		}
...
unsigned xzynq_bdring_from_hw_rx(xzynq_bdring_t *ring_ptr, unsigned bd_limit,
		xzynq_bd_t **bdset_ptr)
{
	xzynq_bd_t *cur_bd_ptr;
	//uint32_t bd_str = 0;
	...
	while (bd_count < bd_limit) {
		...
		if (!(xzynq_bd_is_rx_new(cur_bd_ptr)))
			break;
		...
		xzynq_bd_t *bdset_ptr)
{
	...
	/* Make sure we are in sync with xzynq_bdringFromHw() */
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/lib/io-pkt/sys/dev_qnx/xzynq/bdring.c $ $Rev: 752035 $")
#endif
