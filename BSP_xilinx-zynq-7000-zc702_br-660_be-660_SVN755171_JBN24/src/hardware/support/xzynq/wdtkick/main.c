...
int main(int argc, char *argv[]) 
{
	...
	hwi_off = hwi_find_device(XZYNQ_HWI_WDT, 0);
	...
	if (!(in32(base + XZYNQ_WDT_ZMR_OFFSET) & XZYNQ_WDT_ZMR_WDEN_MASK)) 
	{
	...
	while (1) {
		out32(base + XZYNQ_WDT_RESTART_OFFSET, XZYNQ_WDT_RESTART_KEY_VAL);
		delay(time);
	}
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/hardware/support/xzynq/wdtkick/main.c $ $Rev: 752035 $")
#endif
