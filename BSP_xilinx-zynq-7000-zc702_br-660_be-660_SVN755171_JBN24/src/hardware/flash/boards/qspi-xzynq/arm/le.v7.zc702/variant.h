...
#define MAN_ID				0x20
#define NUM_ERASE_UNITS     TOTAL_SIZE
#define PWR2_SIZE_UNIT		16

#define TOTAL_SIZE_BYTES	(TOTAL_SIZE * SECTOR_SIZE * PAGE_SIZE)

#define MAX_BURST			(XZYNQ_QSPI_FIFO_DEPTH*XZYNQ_QSPI_FIFO_WIDTH)

#endif

//each returns EOK if success, -1 on error, with errno set 
int qspi_cmd_read(int fd, uint8_t cmd[4], uint8_t* buf, int len);
int qspi_cmd_write(int fd, uint8_t cmd[4], uint8_t const* buf, int len);
int qspi_nor_flash_write(int fd, uint8_t const* buf, int len);
int qspi_write_1byte(int fd, uint8_t cmd);
int qspi_word_exchange(int fd, uint32_t* buf);
