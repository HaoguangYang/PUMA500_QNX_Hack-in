...
static inline int
read_sect(xzynq_sdmmc_t *sdmmc, unsigned blkno, void *buf, unsigned blkcnt)
{
...
{
    return read_sect((xzynq_sdmmc_t *)fs_info.device,
...
int
fat_read_mbr(xzynq_sdmmc_t *sdmmc, int verbose)
{
    mbr_t           *mbr;
    bpb_t           *bpb;
    partition_t     *pe;
    ...
    /* In case of FAT partition formatted by Windows */
    if ((GET_LONG((unsigned char*)mbr + 0x52) & 0xFFFFFF) == 0x544146) {
        pe = (partition_t *)mbr;
        fs_info.fs_offset = 0;
    } else {
