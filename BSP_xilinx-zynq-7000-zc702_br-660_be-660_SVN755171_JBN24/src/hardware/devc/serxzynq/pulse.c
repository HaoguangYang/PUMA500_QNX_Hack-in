...
struct my_pulse_struct
{
    ...
    void     (*handler) (DEV_XZYNQ * dev, struct sigevent * event);
    DEV_XZYNQ *dev;
};
...
my_attach_pulse (void **x, struct sigevent *event,
    void (*handler) (DEV_XZYNQ * dev, struct sigevent * event), DEV_XZYNQ * dev)
{
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/hardware/devc/serxzynq/pulse.c $ $Rev: 752035 $")
#endif
