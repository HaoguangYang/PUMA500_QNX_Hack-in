...
// Function prototypes
void mdriver_print_data(CANDEV_XZYNQ_INFO *devinfo);
...
int mdriver_init(CANDEV_XZYNQ_INFO *devinfo, CANDEV_XZYNQ_INIT *devinit)
{
	struct mdriver_entry *mdriver =
			(struct mdriver_entry *) SYSPAGE_ENTRY(mdriver);
	int num_mdrivers = 0;
	int mdriver_intr = -1;
	int mindex;
	...
	if (mdriver_intr != -1) {
		...
		if ((devinfo->mdata = (MINICAN_DATA *) mmap_device_memory(0,
				mdriver[mindex].data_size, PROT_READ | PROT_WRITE
						| PROT_NOCACHE, 0, mdriver[mindex].data_paddr)) == NULL) {
			perror("Can't map mini-driver data");
			exit(EXIT_FAILURE);
		}
		// Buffered messages of type CAN_MSG_OBJ follow the MINICAN_DATA data structure in memory
		devinfo->mcanmsg = (CAN_MSG_OBJ *) ((uint8_t *) devinfo->mdata
				+ sizeof(*devinfo->mdata));
#ifdef DEBUG_MDRVR
 ...
 */
int mdriver_find_mbxid(CANDEV_XZYNQ_INFO *devinfo, uint32_t canmid)
{
	int i;

	// Search through all receive mailboxes to find a matching message ID
	for (i = 0; i < devinfo->numrx; i++) {
		if (canmid == devinfo->canmsg[i].id) {
			return (i);
		}
	}
...
void mdriver_init_data(CANDEV_XZYNQ_INFO *devinfo, CANDEV_XZYNQ_INIT *devinit)
{
	...
	for (i = mdata->nrx - 1; i > -1; i--) {
		if (sort)
			...
			mbxid = mdriver_find_mbxid(devinfo, mcanmsg[i].id);
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/hardware/can/xzynq/mdriver.c $ $Rev: 752035 $")
#endif
