 ...
 * C source code generated on : Tue Nov 14 21:04:59 2017
 *
...
static void Template_output0(void)     /* Sample time: [0.04s, 0.0s] */
{
  ...
  //================Put your periodic computation block here.=====================
...
static void Template_output1(void)     /* Sample time: [1.0s, 0.0s] */
{
  ...
  /* Level2 S-Function Block: '<Root>/Power On' (robot_power) */
  {
    SimStruct *rts = Template_M->childSfunctions[1];
    sfcnOutputs(rts, 1);
  }
}
...
RT_MODEL_Template_T *Template(void)
{
  ...
  /* Initialize timing info */
  {
    ...
    /* task periods */
    Template_M->Timing.sampleTimes[0] = (0.04);
    Template_M->Timing.sampleTimes[1] = (1.0);
  ...
  rtmSetTFinal(Template_M, 120.0);
  Template_M->Timing.stepSize0 = 0.04;
  Template_M->Timing.stepSize1 = 1.0;
  ...
  /* Setup for data logging */
  {
    ...
    rtliSetLogT(Template_M->rtwLogInfo, "tout");
  ...
  /* register each child */
  {
    ...
    Template_M->childSfunctions[1] =
      (&Template_M->NonInlinedSFcns.childSFunctions[1]);
    ...
    /* Level2 S-Function Block: Template/<Root>/Power On (robot_power) */
    {
      SimStruct *rts = Template_M->childSfunctions[1];

      /* timing info */
      time_T *sfcnPeriod = Template_M->NonInlinedSFcns.Sfcn1.sfcnPeriod;
      time_T *sfcnOffset = Template_M->NonInlinedSFcns.Sfcn1.sfcnOffset;
      int_T *sfcnTsMap = Template_M->NonInlinedSFcns.Sfcn1.sfcnTsMap;
      (void) memset((void*)sfcnPeriod, 0,
                    sizeof(time_T)*1);
      (void) memset((void*)sfcnOffset, 0,
                    sizeof(time_T)*1);
      ...
      /* Set up the mdlInfo pointer */
      {
        ssSetBlkInfo2Ptr(rts, &Template_M->NonInlinedSFcns.blkInfo2[1]);
      }
      ...
      /* Allocate memory of model methods 2 */
      {
        ssSetModelMethods2(rts, &Template_M->NonInlinedSFcns.methods2[1]);
      }

      /* Allocate memory of model methods 3 */
      {
        ssSetModelMethods3(rts, &Template_M->NonInlinedSFcns.methods3[1]);
      }

      /* Allocate memory for states auxilliary information */
      {
        ssSetStatesInfo2(rts, &Template_M->NonInlinedSFcns.statesInfo2[1]);
        ssSetPeriodicStatesInfo(rts,
          &Template_M->NonInlinedSFcns.periodicStatesInfo[1]);
      }

      /* path info */
      ssSetModelName(rts, "Power On");
      ssSetPath(rts, "Template/Power On");
      ssSetRTModel(rts,Template_M);
      ssSetParentSS(rts, (NULL));
      ssSetRootSS(rts, rts);
      ssSetVersion(rts, SIMSTRUCT_VERSION_LEVEL2);
  ...
  Template_M->Sizes.numBlocks = (2);   /* Number of blocks */
